...
SPEED        = 0.40               # モータON時の速度(0..1)
CLAMP        = 0.85               # 安全のための上限
WHITE_IS_HIGH = True              # 「白で値が大きく、黒で小さい」なら True
THRESH       = 0.60               # 閾値（WHITE_IS_HIGH=True前提の例）
                                     # 黒で大きく白で小さい場合は WHITE_IS_HIGH=False とし、THRESHは0.40などに

PRINT_EVERY  = 0.5                # 何秒ごとにログを出すか
DT           = 0.02               # ループ周期（50Hz）
...
def main():
    ...
    print("=== sensor_to_motors_basic: start (Ctrl+C to stop) ===")
