...
mbd_phs5 の制御アルゴリズムを実機用に移植し、タクトスイッチで
複数プログラムを切り替えられるようにした構成です。
...
走行速度やセンサーしきい値は `tuning.py` / `tuning_alt.py` で一括調整できます。
...
```
mbd_phs5_switch/
├── program_selector.py       # タクトスイッチで切替・起動
├── real_line_follower.py     # 基本チューニング（安定寄り）
├── real_line_follower_alt.py # タイトコース向けチューニング
├── real_line_follower_with_button.py  # 単体運用（開始/停止）
├── tuning.py                 # 基本チューニング
├── tuning_alt.py             # タイトコース向けチューニング
...
```bash
python3 program_selector.py
```

**操作**:
- 短押し: 候補切替 / 走行中は停止
- 長押し(1.5秒以上): 走行開始 / 走行中は停止
- 超長押し(4.0秒以上): シャットダウン

※ ディスプレイを使うシミュレーター機能は含まれていません。
