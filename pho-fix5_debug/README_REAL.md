...
mbd_phs5 系の制御を実機向けに調整し、フォトリフレクタの最大値が 0.45 程度に下がる環境でも動くよう正規化を追加したプログラムです。
...
├── real_line_follower_with_button.py # タクトスイッチ開始/停止
├── diagnose_button.py                # ボタン診断ログ
├── diagnose_motors.py                # モーター診断ログ
├── diagnose_sensors.py               # センサー診断ログ
├── tuning.py                         # センサー正規化と走行パラメータ
├── test_sensors.py                   # センサー動作確認（RAW/NORM）
...
ディスプレイ接続で原因特定する場合は、以下を実行してください。
ログは `logs/` に保存され、コンソールにも出力されます。

```bash
python3 diagnose_button.py
python3 diagnose_motors.py
python3 diagnose_sensors.py
```
